...
export function parseSettings(data: unknown): GLinkSettings {
  ...
  return {
    automaticLinking: storedSettings.automaticLinking === true,
    darkMode: storedSettings.darkMode === true,
    darkFilter: {
      invert: parseFilterValue(storedFilter.invert, DEFAULT_DARK_FILTER.invert, 70, 100),
      hue: parseFilterValue(storedFilter.hue, DEFAULT_DARK_FILTER.hue, 0, 360),
