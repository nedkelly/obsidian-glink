...
import { getLinkAutomatically, type AutomaticLinkResult } from "./automatic-linker";
...
import { DEFAULT_SETTINGS, parseSettings, type GLinkSettings } from "./plugin-settings";
...
export default class GLinkPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.registry = LinkRegistry.fromData(loaded, (data) => this.persistData(data));

    this.registerView(VIEW_TYPE_GLINK, (leaf) => new GLinkView(leaf, this));
  ...
  refreshViews(path?: string): void {
    this.app.workspace.iterateAllLeaves((leaf) => {
      if (leaf.view instanceof GLinkView && (!path || leaf.view.file?.path === path)) {
        leaf.view.refresh();
      }
    });
  }
