...
export async function getLinkAutomatically(
  ...
  if (!Platform.isWin) {
    return {
      ok: false,
      error: "Automatic linking is only available on Windows.",
    };
  }
  ...
  if (!copied) {
    return {
      ok: false,
      error: "Google Drive did not copy a new link. Paste the URL manually instead.",
    };
  }
...
function runCopyLinkVerb(targetPath: string): Promise<void> {
  const encodedCommand = Buffer.from(COPY_LINK_SCRIPT, "utf16le").toString("base64");
  ...
  return new Promise((resolve, reject) => {
    execFile(
      ...
      (error, _stdout, stderr) => {
        ...
        reject(
          new Error(
            detail || "Windows could not invoke Google Drive’s copy-link action.",
          ),
        );
      },
    );
